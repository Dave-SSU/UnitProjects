 ...
 * $Id: common.h,v 1.63 1999/08/16 02:14:42 mann Exp $
 *
...
typedef enum {
    ...
    ColorTell, ColorChallenge, ColorRequest, ColorNormal, ColorNone, 
    NColorClasses
} ColorClass;
...
typedef enum {
    ...
    VariantLosers,       /* Win by losing all pieces or getting mated (ICC) */
    VariantSuicide,      /* Win by losing all pieces including king (FICS) */
    VariantTwoKings,     /* Weird ICC wild 9 */
    VariantKriegspiel,   /* Kriegspiel, no special support provided */
    VariantAtomic        /* Possible future extension */
} VariantClass;
...
typedef struct {
    ...
    char *xpmDirectory; /* Path to XPM files to use (xboard only) */
