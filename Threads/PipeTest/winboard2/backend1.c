 ...
 * XBoard $Id: backend.c,v 1.116 1998/06/21 01:35:36 mann Exp $
 *
...
int cmailMoveType[CMAIL_MAX_GAMES], ics_bughouse = FALSE, ics_suicide = FALSE;
...
#define STARTED_COMMENT 6
	...
	while (i < buf_len) {
		...
		/* Kludge for suicide chess.  There should be a better way to
		   know which rules to use. */
		if (StrCaseStr(gameInfo.event, "suicide") != NULL) {
		    ics_suicide = TRUE;
		}
		continue;
	    }
     ...
     ChessProgramState *cps;
{
	...
	if (cps == &first && hintRequested) {
	    hintRequested = FALSE;
	    if (ParseMachineMove(buf1, forwardMostMove, &moveType,
				 &fromX, &fromY, &toX, &toY, &promoChar)) {
     ...
     int redraw;
{
    ...
    ics_getting_history = H_FALSE;
    ics_gamenum = -1;
    ics_bughouse = ics_suicide = FALSE;
