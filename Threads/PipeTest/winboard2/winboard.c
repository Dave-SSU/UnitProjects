...
#define WM_MOUSELEAVE WM_USER_Mouseleave
#endif
...
static RECT boardRect, messageRect, whiteRect, blackRect;
...
HWND hwndMain = 0;        /* root window*/
static HWND hwndPause;    /* pause button */
static HWND hCommPort;    /* currently open comm port */
static HBITMAP pieceBitmap[2][(int) WhiteKing + 1];
static HBRUSH lightSquareBrush, darkSquareBrush,
  whitePieceBrush, blackPieceBrush, iconBkgndBrush;
...
static COLORREF lightSquareColor, darkSquareColor, 
  whitePieceColor, blackPieceColor, highlightSquareColor;
...
static char *defaultTextAttribs[] = 
{
  ...
  COLOR_TELL, COLOR_CHALLENGE, COLOR_REQUEST, COLOR_NORMAL, COLOR_NONE
};
...
static MySound soundMove, soundBell;
...
static ColorClass currentColorClass;
...
VOID SoundPopup(HWND hwnd, MySound *ms);
	...
	LPSTR lpCmdLine, int nCmdShow)
{
  ...
  while (GetMessage(&msg, /* message structure */
		    ...
		    0))   /* highest message to examine */
    {
	  ...
	  !TranslateAccelerator(hwndMain, hAccelMain, &msg) &&
...
ArgDescriptor argDescriptors[] = {
  ...
  { "firstChessProgram", ArgFilename, (LPVOID) &appData.firstChessProgram, FALSE },
  { "fcp", ArgFilename, (LPVOID) &appData.firstChessProgram, FALSE },
  { "secondChessProgram", ArgFilename, (LPVOID) &appData.secondChessProgram, FALSE },
  ...
  { "soundMove", ArgFilename, (LPVOID) &soundMove.name, TRUE },
  { "soundBell", ArgFilename, (LPVOID) &soundBell.name, TRUE },
...
void ParseAttribs(ColorClass cc, char* argValue)
{
  char *e = argValue;
  int effects = 0;

  while (*e) {
    if (*e == 'b') effects |= CFE_BOLD;
    else if (*e == 'i') effects |= CFE_ITALIC;
    else if (*e == 'u') effects |= CFE_UNDERLINE;
    else if (*e == 's') effects |= CFE_STRIKEOUT;
    else if (*e == '#' || isdigit(*e)) break;
    e++;
  }
  textAttribs[cc].effects = effects;
  textAttribs[cc].color = ParseColorName(e);
}
...
#define ARG_MAX 10000
  ...
  for (;;) {
    ...
    *q = NULLCHAR;
    ...
    case ArgString:
    case ArgFilename:
      *(char **) ad->argLoc = _strdup(argValue);
      break;
	...
	if (ParseSettingsFile(argValue, fullname)) {
	  if (ad->argLoc != NULL) {
	    *(char **) ad->argLoc = _strdup(fullname);
	  }
	} else {
	  if (ad->argLoc != NULL) {
	  } else {
	...
	*(Boolean *) ad->argLoc = FALSE;
    ...
    case ArgAttribs:
      ParseAttribs((ColorClass)ad->argLoc, argValue);
      break;
...
SetDefaultTextAttribs()
{
  ...
  for (cc = (ColorClass)0; cc < NColorClasses; cc++) {
    ParseAttribs(cc, defaultTextAttribs[cc]);
  }
}
...
SetDefaultSounds()
{
  ColorClass cc;
  for (cc = (ColorClass)0; cc < NColorClasses; cc++) {
    textAttribs[cc].sound.name = _strdup("");
    textAttribs[cc].sound.data = NULL;
  }
  soundBell.name = _strdup(SOUND_BELL);
  soundMove.name = _strdup(SOUND_MOVE);
}
...
LoadAllSounds()
{
  ...
  MyLoadSound(&soundBell);
  MyLoadSound(&soundMove);
}
...
{
  int i, j;
  char buf[MSG_SIZ], currDir[MSG_SIZ];
  char *dummy;
  ...
  ParseIcsTextMenu(icsTextMenuString);
}
...
InitMenuChecks()
{
  ...
  (void) CheckMenuItem(hmenu, IDM_AlwaysOnTop,
		       MF_BYCOMMAND|(alwaysOnTop ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_AlwaysQueen,
		       MF_BYCOMMAND|(appData.alwaysPromoteToQueen ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Animate,
		       MF_BYCOMMAND|(appData.animate ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Dragging,
		       MF_BYCOMMAND|(appData.animateDragging ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_AutoComment,
		       MF_BYCOMMAND|(appData.autoComment ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Autoflag,
		       MF_BYCOMMAND|(appData.autoCallFlag ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Autobs,
		       MF_BYCOMMAND|(appData.autoObserve ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Bell,
		       MF_BYCOMMAND|(appData.ringBellAfterMoves ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_DontColorize,
		       MF_BYCOMMAND|(appData.colorize ?
				     MF_UNCHECKED : MF_CHECKED));
  (void) CheckMenuItem(hmenu, IDM_GetMoveList,
		       MF_BYCOMMAND|(appData.getMoveList ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_HighlightDragging,
		       MF_BYCOMMAND|(appData.highlightDragging ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_HighlightLastMove,
		       MF_BYCOMMAND|(appData.highlightLastMove ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_LocalEdit,
		       MF_BYCOMMAND|(appData.localLineEditing ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_Monochrome,
		       MF_BYCOMMAND|(appData.monoMode ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_PeriodicUpdates,
		       MF_BYCOMMAND|(appData.periodicUpdates ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_PonderNextMove,
		       MF_BYCOMMAND|(appData.ponderNextMove ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_PopupMoveErrors,
		       MF_BYCOMMAND|(appData.popupMoveErrors ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_QuietPlay,
		       MF_BYCOMMAND|(appData.quietPlay ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_ShowCoords,
		       MF_BYCOMMAND|(appData.showCoords ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_ShowThinking,
		       MF_BYCOMMAND|(appData.showThinking ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, IDM_TestLegality,
		       MF_BYCOMMAND|(appData.testLegality ?
				     MF_CHECKED : MF_UNCHECKED));
  ...
  (void) CheckMenuItem(hmenu, IDM_SaveSettingsOnExit,
		       MF_BYCOMMAND|(saveSettingsOnExit ?
				     MF_CHECKED : MF_UNCHECKED));
  (void) CheckMenuItem(hmenu, boardSize + IDM_Tiny, MF_BYCOMMAND|MF_CHECKED);
}
...
SaveSettings(char* name)
{
  ...
  fprintf(f, ";\n");
  fprintf(f, "; WinBoard settings file, written by Save Settings command\n");
  fprintf(f, ";\n");
  fprintf(f, "; You can edit existing settings in this file, but if you add new\n");
  fprintf(f, "; ones, the next Save Settings will not save them.  Put any additional\n");
  fprintf(f, "; settings in a shortcut, an @indirection file, or a .bat file.\n");
  fprintf(f, ";\n");
...
ResizeBoard(int newSizeX, int newSizeY, int flags)
{
  ...
  (void) CheckMenuItem(GetMenu(hwndMain), boardSize + IDM_Tiny,
...
InitDrawingSizes(BoardSize boardSize, int flags)
{
  ...
  for (i = 0; i < N_BUTTONS; i++) {
	...
	(WPARAM)font[boardSize][MESSAGE_FONT]->hf, MAKELPARAM(FALSE, 0));
    }
...
VOID
DrawHighlightOnDC(HDC hdc, BOOLEAN on, int x, int y)
{
  int x1, y1;
  HPEN oldPen;
  if (lineGap == 0) return;
  ...
  oldPen = SelectObject(hdc, on ? highlightPen : gridPen);
...
DrawPieceOnDC(HDC hdc, ChessSquare piece, int color, int sqcolor, int x, int y, HDC tmphdc)
{
  ...
  if (appData.monoMode) {
    ...
    oldBrush = SelectObject(hdc, color ? whitePieceBrush : blackPieceBrush);
...
SetupDropMenu(HMENU hmenu)
{
  ...
  for (i=0; i<sizeof(dropEnables)/sizeof(DropEnable); i++) {
    ...
    ModifyMenu(hmenu, dropEnables[i].command,
	       MF_BYCOMMAND|(count > 0 ? MF_ENABLED : MF_GRAYED)|MF_STRING,
...
MouseEvent(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_MOUSEMOVE:
	...
	SetHighlights(fromX, fromY, x, y);
      ...
      if (timer == -1) {
        timer = SetTimer(hwndMain, MOUSE_TIMER_ID, 50, NULL);
      }
    } else {
      if (timer != -1) {
        KillTimer(hwndMain, MOUSE_TIMER_ID);
        timer = -1;
      }
    }
    break;
...
ToggleShowThinking()
{
  ...
  (void) CheckMenuItem(GetMenu(hwndMain), IDM_ShowThinking,
		       MF_BYCOMMAND|(appData.showThinking ?
		                     MF_CHECKED : MF_UNCHECKED));
}
...
WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND: /* message: command from application menu */
    ...
    case IDM_AnalysisMode:
      if (!analysisSupport) {
        char buf1[MSG_SIZ], buf2[MSG_SIZ];
        TidyProgramName(appData.firstChessProgram, appData.firstHost, buf1);
        sprintf(buf2, "%s does not support analysis", buf1);
        DisplayError(buf2, 0);
      } else {
    ...
    case IDM_AlwaysOnTop:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_AlwaysOnTop,
			   MF_BYCOMMAND|(alwaysOnTop ?
					 MF_CHECKED : MF_UNCHECKED));
      SetWindowPos(hwndMain, alwaysOnTop ? HWND_TOPMOST : HWND_NOTOPMOST,
		   0, 0, 0, 0, SWP_NOMOVE|SWP_NOSIZE);
    ...
    case IDM_AlwaysQueen:
      appData.alwaysPromoteToQueen = !appData.alwaysPromoteToQueen;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_AlwaysQueen,
			   MF_BYCOMMAND|(appData.alwaysPromoteToQueen ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_AutoComment:
      appData.autoComment = !appData.autoComment;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_AutoComment,
			   MF_BYCOMMAND|(appData.autoComment ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_Autoflag:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Autoflag,
			   MF_BYCOMMAND|(appData.autoCallFlag ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_Autobs:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Autobs,
			   MF_BYCOMMAND|(appData.autoObserve ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_Animate:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Animate,
			   MF_BYCOMMAND|(appData.animate ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_Dragging:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Dragging,
			   MF_BYCOMMAND|(appData.animateDragging ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_Bell:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Bell,
			   MF_BYCOMMAND|(appData.ringBellAfterMoves ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_GetMoveList:
      appData.getMoveList = !appData.getMoveList;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_GetMoveList,
			   MF_BYCOMMAND|(appData.getMoveList ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_HighlightDragging:
      appData.highlightDragging = !appData.highlightDragging;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_HighlightDragging,
			   MF_BYCOMMAND|(appData.highlightDragging ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_HighlightLastMove:
      appData.highlightLastMove = !appData.highlightLastMove;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_HighlightLastMove,
			   MF_BYCOMMAND|(appData.highlightLastMove ?
					 MF_CHECKED : MF_UNCHECKED));
      if (!appData.highlightLastMove) {
	ClearHighlights();
    ...
    case IDM_LocalEdit:
      appData.localLineEditing = !appData.localLineEditing;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_LocalEdit,
			   MF_BYCOMMAND|(appData.localLineEditing ?
					 MF_CHECKED : MF_UNCHECKED));
    ...
    case IDM_PeriodicUpdates:
      PeriodicUpdatesEvent(!appData.periodicUpdates);
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_PeriodicUpdates,
			   MF_BYCOMMAND|(appData.periodicUpdates ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_PonderNextMove:
      PonderNextMoveEvent(!appData.ponderNextMove);
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_PonderNextMove,
			   MF_BYCOMMAND|(appData.ponderNextMove ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_PopupMoveErrors:
      appData.popupMoveErrors = !appData.popupMoveErrors;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_PopupMoveErrors,
			   MF_BYCOMMAND|(appData.popupMoveErrors ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_QuietPlay:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_QuietPlay,
			   MF_BYCOMMAND|(appData.quietPlay ?
					 MF_CHECKED : MF_UNCHECKED));
      break;
    ...
    case IDM_ShowCoords:
      ...
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_ShowCoords,
			   MF_BYCOMMAND|(appData.showCoords ?
					 MF_CHECKED : MF_UNCHECKED));
      InvalidateRect(hwnd, &boardRect, FALSE);
      break;

    case IDM_ShowThinking:
      ShowThinkingEvent(!appData.showThinking);
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_ShowThinking,
			   MF_BYCOMMAND|(appData.showThinking ?
					 MF_CHECKED : MF_UNCHECKED));
    ...
    case IDM_TestLegality:
      appData.testLegality = !appData.testLegality;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_TestLegality,
			   MF_BYCOMMAND|(appData.testLegality ?
					 MF_CHECKED : MF_UNCHECKED));
      break;

    case IDM_CommPort:
      lpProc = MakeProcInstance((FARPROC)CommPortOptions, hInst);
      DialogBox(hInst, MAKEINTRESOURCE(DLG_CommPort), hwnd,
		(DLGPROC) lpProc);
      FreeProcInstance(lpProc);
      break;

    case IDM_LoadOptions:
      lpProc = MakeProcInstance((FARPROC)LoadOptions, hInst);
      DialogBox(hInst, MAKEINTRESOURCE(DLG_LoadOptions), hwnd,
		(DLGPROC) lpProc);
      FreeProcInstance(lpProc);
      break;

    case IDM_SaveOptions:
      lpProc = MakeProcInstance((FARPROC)SaveOptions, hInst);
      DialogBox(hInst, MAKEINTRESOURCE(DLG_SaveOptions), hwnd,
		(DLGPROC) lpProc);
      FreeProcInstance(lpProc);
      break;
	...
	lpProc = MakeProcInstance((FARPROC)TimeControl, hInst);
	DialogBox(hInst, MAKEINTRESOURCE(DLG_TimeControl), hwnd,
	  	(DLGPROC) lpProc);
	FreeProcInstance(lpProc);
      }
      break;
    ...
    case IDM_Colossal:
    case IDM_Titanic:
      ChangeBoardSize(wmId - IDM_Tiny);
      break;

    case IDM_LightSquares:
      ChangeColor(hwnd, &lightSquareColor);
      break;

    case IDM_DarkSquares:
      ChangeColor(hwnd, &darkSquareColor);
      break;

    case IDM_WhitePieces:
      ChangeColor(hwnd, &whitePieceColor);
      break;

    case IDM_BlackPieces:
      ChangeColor(hwnd, &blackPieceColor);
      break;

    case IDM_SquareHighlights:
      ChangeColor(hwnd, &highlightSquareColor);
      InitDrawingSizes(boardSize, 0);
      break;
    ...
    case IDM_DefaultColors:
      ...
      InitDrawingSizes(boardSize, 0);
      appData.monoMode = FALSE;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Monochrome,
  			   MF_BYCOMMAND|(appData.monoMode ?
  					 MF_CHECKED : MF_UNCHECKED));
      InvalidateRect(hwnd, &boardRect, FALSE);
      break;
    ...
    case IDM_Monochrome:
      appData.monoMode = !appData.monoMode;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_Monochrome,
			   MF_BYCOMMAND|(appData.monoMode ?
					 MF_CHECKED : MF_UNCHECKED));
      InvalidateRect(hwnd, &boardRect, FALSE);
    ...
    case IDM_ConsoleBackground:
      ChangeColor(hwnd, &consoleBackgroundColor);
    ...
    case IDM_ColorShout:
    case IDM_ColorSShout:
    case IDM_ColorChannel1:
    case IDM_ColorChannel:
    case IDM_ColorKibitz:
    case IDM_ColorTell:
    case IDM_ColorChallenge:
    case IDM_ColorRequest:
    case IDM_ColorNormal:
      ColorizeTextPopup(hwnd, (ColorClass)(wmId - IDM_ColorShout));
      break;

    case IDM_DefaultColorization:
      appData.colorize = TRUE;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_DontColorize,
			   MF_BYCOMMAND|(appData.colorize ?
					 MF_UNCHECKED : MF_CHECKED));
      {
	CHARFORMAT cf;
    ...
    case IDM_DontColorize:
      appData.colorize = !appData.colorize;
      (void) CheckMenuItem(GetMenu(hwndMain), IDM_DontColorize,
			   MF_BYCOMMAND|(appData.colorize ?
					 MF_UNCHECKED : MF_CHECKED));
      break;
    ...
    case IDM_CoordFont:
      if (MyCreateFont(font[boardSize][COORD_FONT])) {
	if (appData.showCoords)
	  InvalidateRect(hwnd, &boardRect, FALSE);
      }
      break;

    case IDM_CommentsFont:
      if (MyCreateFont(font[boardSize][COMMENT_FONT])) {
	if (commentDialog) {
	  SendMessage(GetDlgItem(commentDialog, OPT_CommentText),
	    WM_SETFONT, (WPARAM)font[boardSize][COMMENT_FONT]->hf, MAKELPARAM(TRUE, 0));
	}
      }
      break;

    case IDM_TagsFont:
      if (MyCreateFont(font[boardSize][EDITTAGS_FONT])) {
	if (editTagsDialog) {
	  SendMessage(GetDlgItem(editTagsDialog, OPT_TagsText),
	    WM_SETFONT, (WPARAM)font[boardSize][EDITTAGS_FONT]->hf, MAKELPARAM(TRUE, 0));
	}
      }
      break;

    case IDM_ConsoleFont:
      if (hwndConsole) {
	if (MyCreateFont(font[boardSize][CONSOLE_FONT])) {
          ChangedConsoleFont();
	}
      }
      break;

    case IDM_DefaultFonts:
      for (i=0; i<NUM_FONTS; i++)
	for (j=0; j<NUM_SIZES; j++) {
	  DeleteObject(&font[j][i]->hf);
          ParseFontName(font[j][i]->def, &font[j][i]->mfp);
	  CreateFontInMF(font[j][i]);
	}
      InitDrawingSizes(boardSize, 0);
    ...
    case IDM_SoundMove:
      SoundPopup(hwnd, &soundMove);
      break;

    case IDM_SoundBell:
      SoundPopup(hwnd, &soundBell);
      break;

    case IDM_SoundShout:
    case IDM_SoundSShout:
    case IDM_SoundChannel1:
    case IDM_SoundChannel:
    case IDM_SoundKibitz:
    case IDM_SoundTell:
    case IDM_SoundChallenge:
    case IDM_SoundRequest:
      SoundPopup(hwnd, &textAttribs[(int)(wmId - IDM_SoundShout)].sound);
      break;

    case IDM_DefaultSounds:
      SetDefaultSounds();
      LoadAllSounds();
      break;
  ...
  case WM_TIMER:
	...
	if (mousepos.x < boardRect.left ||
            mousepos.x > boardRect.right ||
            mousepos.y < boardRect.top ||
            mousepos.y > boardRect.bottom) {
    ...
    case LOAD_GAME_TIMER_ID:
      ...
      LoadGameLoop(); /* call into back end */
      break;
...
TimeControl(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_COMMAND: /* message: received a command */
	...
	if (!ParseTimeControl(buf, increment, mps)) {
      ...
      appData.timeControl = _strdup(buf);
...
LRESULT CALLBACK
SaveOptions(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  char buf[MSG_SIZ];
  FILE *f;
...
LRESULT CALLBACK
CommPortOptions(HWND hDlg, UINT message, WPARAM wParam,	LPARAM lParam)
{
  char buf[MSG_SIZ];
  HANDLE hwndCombo;
  char *p;
  ...
  case WM_COMMAND: /* message: received a command */
	...
	return TRUE;
      ...
      appData.icsCommPort = _strdup(buf);
      if (hCommPort != NULL) {
	CloseHandle(hCommPort);
	hCommPort = NULL;
      }
...
void
InitComboStringsFromString(HANDLE hwndCombo, char *str)
{
  SendMessage(hwndCombo, CB_RESETCONTENT, 0, 0);
...
InitEngineBox(HWND hDlg, HWND hwndCombo, char* nthcp, char* nthd, char* nthdir, char *nthnames)
{
  ...
  InitComboStringsFromString(hwndCombo, nthnames);
...
StartupDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    InitComboStringsFromString(hwndCombo, icsNames);    
...
VOID
UpdateSampleText(HWND hDlg, MyTextAttribs* mta)
{
  CHARFORMAT cf;
  cf.cbSize = sizeof(CHARFORMAT);
  cf.dwMask = 
    CFM_COLOR|CFM_BOLD|CFM_ITALIC|CFM_UNDERLINE|CFM_STRIKEOUT|CFM_FACE|CFM_SIZE;
  cf.crTextColor = mta->color;
  cf.dwEffects = mta->effects;
  ...
  SendDlgItemMessage(hDlg, OPT_Sample, EM_SETCHARFORMAT, SCF_ALL, (LPARAM)&cf);
}
...
ColorizeTextDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_INITDIALOG:
    ...
    CenterWindow (hDlg, GetWindow (hDlg, GW_OWNER));
    /* Initialize the dialog items */
    CheckDlgButton(hDlg, OPT_Bold, (textAttribs[cc].effects & CFE_BOLD) != 0);
    CheckDlgButton(hDlg, OPT_Italic, (textAttribs[cc].effects & CFE_ITALIC) != 0);
    CheckDlgButton(hDlg, OPT_Underline, (textAttribs[cc].effects & CFE_UNDERLINE) != 0);
    CheckDlgButton(hDlg, OPT_Strikeout, (textAttribs[cc].effects & CFE_STRIKEOUT) != 0);
    SendDlgItemMessage(hDlg, OPT_Sample, 
      EM_SETBKGNDCOLOR, FALSE, consoleBackgroundColor);
    SetDlgItemText(hDlg, OPT_Sample, "Sample Text");
    UpdateSampleText(hDlg, &mta);
    return TRUE;
...
LRESULT CALLBACK
SoundDialog(HWND hDlg, UINT message, WPARAM wParam, LPARAM lParam)
{
  static MySound *ms;
  int radio;
  MySound tmp;
  ...
  case WM_INITDIALOG:
	...
	radio = OPT_WavFile;
	SetDlgItemText(hDlg, OPT_WavFileName, ms->name);
      }
      break;
    case '!':
      if (ms->name[1] == NULLCHAR) {
	...
	radio = OPT_BuiltInSound;
	if (SendMessage(hBISN, CB_SELECTSTRING, (WPARAM) -1, (LPARAM) (ms->name + 1)) == CB_ERR) {
          SendMessage(hBISN, CB_SETCURSEL, (WPARAM) -1, (LPARAM) 0);
	  SendMessage(hBISN, WM_SETTEXT, (WPARAM) 0, (LPARAM) (ms->name + 1));
	}
      }
      break;
    default:
  ...
  case WM_COMMAND: /* message: received a command */
    ...
    case IDOK:
      ...
      free(ms->name);
      ms->name = _strdup(SoundDialogGetName(hDlg, radio));
      MyLoadSound(ms);
      EndDialog(hDlg, TRUE);
      return TRUE;
...
VOID
SoundPopup(HWND hwnd, MySound *ms)
{
  FARPROC lpProc;

  lpProc = MakeProcInstance((FARPROC)SoundDialog, hInst);
  DialogBoxParam(hInst, MAKEINTRESOURCE(DLG_Sound),
    hwnd, (DLGPROC)lpProc, (LPARAM) ms);
  FreeProcInstance(lpProc);
}
...
ErrorPopDown()
{
  if (!appData.popupMoveErrors) DisplayMessage("", "");
...
DisplayFatalError(char *str, int error, int exitStatus)
{
  ...
  if (error == 0) {
    (void) MessageBox(hwndMain, str, "Fatal Error", MB_OK|MB_ICONSTOP);
  } else {
    len = FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM,
			NULL, error, LANG_NEUTRAL,
			(LPSTR) buf2, MSG_SIZ, NULL);
    ...
    (void) MessageBox(hwndMain, buf, "Fatal Error", MB_OK|MB_ICONSTOP);
  }
  if (exitStatus != -1) exit(exitStatus);
}
...
DisplayTitle(char *str)
{
  ...
  if (str[0] != NULLCHAR) {
    ...
    strcpy(title, szTitle);
    strcat(title, ": ");
    TidyProgramName(appData.firstChessProgram, appData.firstHost, 
      title + strlen(title));
  }
...
Enables gnuEnables[] = {
  ...
  { IDM_AutoComment, MF_BYCOMMAND|MF_GRAYED },
  { IDM_Autobs, MF_BYCOMMAND|MF_GRAYED },
  { IDM_DontColorize, MF_BYCOMMAND|MF_GRAYED },
  { IDM_GetMoveList, MF_BYCOMMAND|MF_GRAYED },
  { IDM_LocalEdit, MF_BYCOMMAND|MF_GRAYED },
  { IDM_QuietPlay, MF_BYCOMMAND|MF_GRAYED },
  { IDM_ConsoleFont, MF_BYCOMMAND|MF_GRAYED },
  { -1, -1 }
};
...
Enables icsEnables[] = {
  ...
  { IDM_SoundBell, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundShout, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundSShout, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundChannel1, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundChannel, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundKibitz, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundTell, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundChallenge, MF_BYCOMMAND|MF_ENABLED },
  { IDM_SoundRequest, MF_BYCOMMAND|MF_ENABLED },
  { IDM_PeriodicUpdates, MF_BYCOMMAND|MF_GRAYED },
  { IDM_MoveNow, MF_BYCOMMAND|MF_GRAYED },
  { IDM_ShowThinking, MF_BYCOMMAND|MF_GRAYED },
  { IDM_PonderNextMove, MF_BYCOMMAND|MF_GRAYED },
...
Enables zippyEnables[] = {
  { IDM_PeriodicUpdates, MF_BYCOMMAND|MF_ENABLED },
  { IDM_MoveNow, MF_BYCOMMAND|MF_ENABLED },
  { IDM_ShowThinking, MF_BYCOMMAND|MF_ENABLED },
  { IDM_PonderNextMove, MF_BYCOMMAND|MF_ENABLED },
...
Enables ncpEnables[] = {
  ...
  { IDM_AutoComment, MF_BYCOMMAND|MF_GRAYED },
  { IDM_Autoflag, MF_BYCOMMAND|MF_GRAYED },
  { IDM_Autobs, MF_BYCOMMAND|MF_GRAYED },
  { IDM_Bell, MF_BYCOMMAND|MF_GRAYED },
  { IDM_DontColorize, MF_BYCOMMAND|MF_GRAYED },
  { IDM_GetMoveList, MF_BYCOMMAND|MF_GRAYED },
  { IDM_LocalEdit, MF_BYCOMMAND|MF_GRAYED },
  { IDM_PeriodicUpdates, MF_BYCOMMAND|MF_GRAYED },
...
SetICSMode()
{
  ...
  EnableMenuItem(GetSubMenu(hmenu, OPTIONS_POS), ICS_COLORS_POS,
...
ConsoleTextSubclass(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
  ...
  case WM_KEYDOWN:
    ...
    case VK_NEXT:
      ...
      SendMessage(hwnd, EM_SETOPTIONS, ECOOP_AND, ~ECO_AUTOVSCROLL);
      return 0;
    }
    break;
  case WM_CHAR:
...
ConsoleOutput(char* data, int length, int forceVisible)
{
  ...
  *q = NULLCHAR;
  ...
  if (forceVisible || exlen == 0 ||
    ...
    sel.cpMax = 9999999;
    SendMessage(hText, EM_EXSETSEL, 0, (LPARAM)&sel);
    SendMessage(hText, EM_SETOPTIONS, ECOOP_OR, ECO_AUTOVSCROLL);
    SendMessage(hText, EM_HIDESELECTION, FALSE, FALSE);
    SendMessage(hText, EM_SCROLLCARET, 0, 0);
    SendMessage(hText, EM_HIDESELECTION, TRUE, FALSE);
    SendMessage(hText, EM_SETOPTIONS, ECOOP_AND, ~ECO_AUTOVSCROLL);
  }
...
#define BUFSIZE 4096
  ...
  *pr = NoProc;
  ...
  if (dir[0] == NULLCHAR) dir = NULL;      /* use parent's current directory */
...
void
DestroyChildProcess(ProcRef pr)
{
  ChildProc *cp;
  ...
  case CPReal:
    ...
    /*!!GenerateConsoleCtrlEvent(CTRL_BREAK_EVENT, cp->pid);*/
...
EchoOff()
{
  ...
  consoleEcho = FALSE;
  hInput = GetDlgItem(hwndConsole, OPT_ConsoleInput);
  /* This works OK: set text and background both to 75% gray */
  cf = consoleCF;
  cf.crTextColor = COLOR_ECHOOFF;
