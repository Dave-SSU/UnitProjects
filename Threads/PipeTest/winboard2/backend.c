 ...
 * XBoard $Id: backend.c,v 1.119 1999/08/16 02:14:42 mann Exp $
 *
...
{
    return
      ((((index % 2) == 0) ? F_WHITE_ON_MOVE : 0) | 
       (gameInfo.variant == VariantSuicide ? F_IGNORE_CHECK : 0) |
       F_ALL_CASTLE_OK);
}
...
InitBackEnd1()
{
    ...
    first.attention = second.attention = TRUE;
     ...
     char *e;
{
    ...
    if (!found) {
      if (StrCaseStr(e, "fischer") || StrCaseStr(e, "fr")) {
        v = VariantFischeRandom;
      } else if ((p = StrCaseStr(e, "wild")) || (p = StrCaseStr(e, "w"))) {
	while (*p && !isdigit(*p)) p++;
	if (*p) {
	  wnum = atoi(p);
	} else {
...
#define STARTED_MOVES_NOHIDE 7
	...
	while (i < buf_len) {
	    ...
	    if (loggedOn && !intfSet) {
		if (ics_ICC) {
		    sprintf(str, "set-quietly interface %s\n", programVersion);
		} else {
		    sprintf(str,
			    "set interface %s\niset startpos 1\niset lock 1\n",
			    programVersion);
		...
		(looking_at(buf, &i, "\"*\" is *a registered name") ||
		 ...
		 looking_at(buf, &i, "Your name will be \"*\""))) {
		...
		if (ZippyControl(buf, &i) ||
		    (appData.zippyTalk && ZippyConverse(buf, &i)) ||
...
#if ZIPPY
		    if (appData.zippyPlay && appData.getMoveList) {
			if (gameMode == IcsPlayingWhite) {
			    FeedMovesToProgram(&first, forwardMostMove);
			    if (WhiteOnMove(forwardMostMove)) {
				if (first.sendTime) {
				    /* Temp set black on move to ensure
				     * "time" is white, "otim" black */
				    SendToProgram("black\n", &first); 
			    ...
			    if (!WhiteOnMove(forwardMostMove)) {
				if (first.sendTime) {
				    /* Temp set white on move to ensure
				     * "time" is black, "otim" white */
				    SendToProgram("white\n", &first);
		...
		*/
		strcpy(player1Name, star_match[0]);
		player1Rating = string_to_rating(star_match[1]);
		strcpy(player2Name, star_match[3]);
		...
		if (started == STARTED_BOARD) {
			...
			gamenum == ics_gamenum) {
			  ...
			  if (appData.debugMode) {
			    fprintf(debugFP, "guessing this is bughouse\n");
			  }
			}
	...
	char *msg = "Connection closed by ICS\n";
	SendToPlayer(msg, strlen(msg));
	ExitEvent(0);
    } else {
 ...
 * The trailing 0 (flip state) is a later addition, and old versions of
 * FICS may not have it. Additional trailing fields may be added in the future.
 */
...
{ 
	...
	if (appData.zippyPlay && !newGame && newMove && 
	   (!appData.getMoveList || backwardMostMove == 0)) {
	...
	/* If move comes from a remote source, animate it. If
	   it isn't remote will have already been animated. */
...
// DMC: start of non-ICS sending functions
void
...
void
SendBoard(cps, board, blackPlaysFirst)
     ChessProgramState *cps;
     Board board;
     int blackPlaysFirst;
{
    ...
    /* Kludge because "black" does not mean "set black on move"
     * for some engines.
     */
    if (blackPlaysFirst) SendToProgram("a2a3\n", cps);
...
HandleMachineMove(message, cps)
     ...
     ChessProgramState *cps;
{
    char machineMove[MSG_SIZ], buf1[MSG_SIZ*4], buf2[MSG_SIZ];
	...
	switch (gameMode) {
	  ...
	  case IcsIdle:
	  default:
	    /* Extra move after we tried to stop.  I'm not sure the mode
	       test is a reliable way of detecting this problem.  We might
	       be out of sync with the machine.  Ugh.  --tpm 
	       */
	    if (appData.debugMode) {
	...
	if (gameMode == TwoMachinesPlay) {
	    ...
	    SendToProgram(machineMove, cps->other);
	    if (firstMove) {
		firstMove = FALSE;
	...
	if (StrStr(message, "name") || StrStr(message, "draw") ||
	...
	if (StrStr(message, "time") || StrStr(message, "otim")) {
	    /* Program doesn't have "time" or "otim" command */
	    cps->sendTime = 0;
	    return;
	}
	if (cps == &first && StrStr(message, "analyze")) {
	...
	if (!ignore) {
		...
		return;
	    } else if (sscanf(message,"stat01: %d %lu %d %d %d",
     ...
     ChessProgramState *cps;
{
	...
	sprintf(cmdLine, "%s %d", cps->program, searchTime);
    } else {
	strcpy(cmdLine, cps->program);
    }

    if (strcmp(cps->host, "localhost") == 0) {
	err = StartChildProcess(cmdLine, cps->dir, &cps->pr);
    } else if (*appData.remoteShell == NULLCHAR) {
	err = OpenRcmd(cps->host, appData.remoteUser, cmdLine, &cps->pr);
    } else {
	if (*appData.remoteUser == NULLCHAR) {
	    sprintf(buf, "%s %s %s", appData.remoteShell, cps->host, cmdLine);
	} else {
	    sprintf(buf, "%s -l %s %s %s", appData.remoteShell,
		    appData.remoteUser, cps->host, cmdLine);
	}
     ...
     int whosays;
{
	...
	if (first.pr != NoProc) {
	    SendToProgram("quit\n", &first);
	    DestroyChildProcess(first.pr);
	}
	first.pr = NoProc;
    }
    if (appData.reuseSecond) {
	...
	if (matchGame < appData.matchGames) {
	    ...
	    /* Kill off chess programs before exiting */
	    if (first.pr != NoProc) {
		SendToProgram("quit\n", &first);
		DestroyChildProcess(first.pr);
	    }
...
/* Assumes program was just reinitialized (initString sent).
     ...
     int upto;
{
	...
	SendBoard(cps, boards[backwardMostMove], backwardMostMove % 2 == 1);
    }
...
LoadGameOneMove()
{
	...
	SendToProgram(currentMoveString, &first);
	if (gameMode == AnalyzeFile) first.maybeThinking = TRUE;
     ...
     int useList;
{
	...
	if (gameInfo.fen == NULL) {
	    ...
	    SendBoard(&first, boards[forwardMostMove], blackPlaysFirst);
	}
	yyboardindex = forwardMostMove;
	cm = (ChessMove) yylex();
    }
     ...
     char *title;
{
    ...
    if (first.pr == NoProc) InitChessProgram(&first);
     ...
     int status;
{
    ...
    if (appData.icsActive && appData.colorize) Colorize(ColorNone);
...
TwoMachinesEvent()
{
	...
	ReinitChessProgram(&second);
    }
    SendToProgram("force\n", &second);
    if (startedFromSetupPosition) {
	SendBoard(&second, boards[backwardMostMove], blackPlaysFirst);
    }
    ...
    SetGameInfo();
    if (appData.matchGames > 0) {
	sprintf(buf, "%s vs. %s (game %d of %d)",
		gameInfo.white, gameInfo.black, matchGame, appData.matchGames);
    } else {
	sprintf(buf, "%s vs. %s", gameInfo.white, gameInfo.black);
    }
    DisplayTitle(buf);
     ...
     int x, y;
{
    ...
    switch (selection) {
      case ClearBoard:
	if (gameMode == IcsExamining && !ics_ICC) {
     ...
     int x, y;
{
	...
	if (pausing) PauseEvent();
	MakeMove((int) selection, DROP_RANK, x, y, NULLCHAR);
	SendMoveToICS(moveType, (int) selection, DROP_RANK, x, y);
	ics_user_moved = 1;
	ShowMove((int) selection, DROP_RANK, x, y);
    }
}
...
ToStartEvent()
{
	 ...
	 * Otherwise Crafty analyzes and prints some output after every
	 * undo, all the way back to the start.
	 */
	SendToProgram(first.initString, &first);
	currentMove=backwardMostMove;
	if (!CompareBoards(boards[currentMove], initialPosition) ||
	    !WhiteOnMove(currentMove)) {
	    SendBoard(&first, boards[currentMove], !WhiteOnMove(currentMove));
	}
	DisplayBothClocks();
	DisplayMove(currentMove - 1);
	ClearHighlights(); /*!!could figure this out*/
...
{
    AskQuestion(title, question, replyPrefix,
		which[0] == '1' ? first.pr : second.pr);
}
...
CheckTimeControl()
{
     ...
     */
    if (movesPerSession && (forwardMostMove % (movesPerSession * 2)) == 0) {
