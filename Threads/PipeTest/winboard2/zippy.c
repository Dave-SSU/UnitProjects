 ...
 * $Id: zippy.c,v 1.52 1999/08/16 02:14:42 mann Exp $
 *
     ...
     int *i;
{
	...
	if ((gameMode == IcsPlayingWhite &&
	    ...
	    SendToICS("say This computer does not play Crafty clones\n");
	    SendToICS("abort\n");
	    sprintf(reply, "+noplay %s\n", player);
	    SendToICS(reply);
	}
	return TRUE;
    }
	...
	if (appData.zippyBughouse > 1) {
	    ...
	    sprintf(reply, "decline %s\n", player);
	    SendToICS(reply);
	} else {
	    sprintf(reply, "tell %s This computer cannot play bughouse\n",
		    player);
	    SendToICS(reply);
	}
	return TRUE;
    ...
    if (appData.zippyPlay && appData.zippyBughouse &&
	...
	if (appData.zippyPassword[0] != NULLCHAR &&
	    ...
	    SendToICS(p);
	    SendToICS("\n");
	} else if (appData.zippyPassword2[0] != NULLCHAR &&
     ...
     int *i;
{
    ...
    /* Messages */
    if (isdigit(buf[*i]) && (looking_at(buf, i, "*. * (*:*): *") ||
			     looking_at(buf, i, "*. * at *:*: *"))) {
	FILE *f;
	char *player = StripHighlightAndTitle(star_match[1]);
	...
	if (strcmp(player, ics_handle) != 0) {
	    ...
	    fprintf(f, "%s (%s:%s): %s\n", player,
		    star_match[2], star_match[3], star_match[4]);
	    fclose(f);
	}
	return TRUE;
    }
    ...
    /* Channel tells */
    oldi = *i;
    if ((ci = 1, looking_at(buf, i, "*(*): *")) ||
        (ci = 2, looking_at(buf, i, "*(*)(*): *"))) {
	char *player;
	...
	if (strcmp(player, ics_handle) != 0 && (random() % 10) < 9) {
	    Speak("tell", star_match[ci]);
	}
#else
	/* Tell to the channel only if someone mentions our name */
	if (ZippyCalled(star_match[ci+1])) {
	    Speak("tell", star_match[ci]);
	}
#endif
	return TRUE;
     ...
     char *srated, *swild, *sbase, *sincrement, *opponent;
{
    ...
    /* If desired, you can insert more code here to decline matches based
       on rated, variant, base, and increment.  Otherwise they are mostly
       not used.  It is probably best to do this with formulas anyway... */

    if ((variant == VariantLoadable) || 
	(StrStr(appData.zippyVariants, varname) == NULL)) {
        sprintf(buf,
	    "tell %s This computer cannot play %s [%s], only %s\ndecline %s\n",
		opponent, swild, varname, appData.zippyVariants, opponent);
	SendToICS(buf);
	return;
    }
	...
	sprintf( buf, "tell %s %s\ndecline %s\n", opponent,
		"Sorry, I'm testing and cannot accept matches now.",
		opponent );
	SendToICS( buf );
	return;
    }
     ...
     int moveNum, basetime, increment;
{
    ...
    if (startedFromSetupPosition || (!appData.getMoveList && moveNum > 0)) {
	SendBoard(&first, boards[moveNum], !WhiteOnMove(moveNum));
    }
	...
	if (appData.getMoveList) {
	    /* Resume from adjournments with move list, to be sent later */
	    if (moveNum == 0) {
		/* Not an adjournment */
		SendToProgram("black\n", &first); /*gnu kludge*/
	    ...
	    /* Resume from adjournments with position only, sent above */
